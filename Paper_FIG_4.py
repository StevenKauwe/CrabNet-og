...
ternaries_dict = defaultdict(list)
quarternaries_dict = defaultdict(list)
for data in model.data_loader.dataset:
    keys = _element_composition(data[2]).keys()
    if len(keys) == 2:
        binaries_dict[tuple(sorted(keys))].append(data)
    if len(keys) == 3:
        ternaries_dict[tuple(sorted(keys))].append(data)
    if len(keys) == 4:
        quarternaries_dict[tuple(sorted(keys))].append(data)
...
for key, systems in binaries_dict.items():
    ...
    with torch.no_grad():
        ...
        plt.xlim(0, 1)
        plt.ylabel("$P_xFe_{1-x} and Others$ p(glass)")
        plt.xlabel("$x$")
