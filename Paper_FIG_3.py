...
def model(mat_prop, classification_list, simple=False):
    ...
    dataset = f"{data_dir}/{mat_prop}/train.csv"
    # dataset = f"{data_dir}/{mat_prop}/val.csv"
        ...
        for key in element_tracker.keys():
            ...
            if len(data) > 2:
                ...
                if plot:
                    ...
                    plt.xlabel(f"{mat_prop} Contribution")
...
def plot(mat_prop, property_tracker):
    ...
    elem_tracker = elem_tracker + pd.Series(property_tracker).drop("None", axis=0) * 100
    ...
    name = mat_prop
    legend_title = "Average Glass Formation Contribution"
    plt.text(
        x_offset + length / 2,
        y_offset + 0.7,
