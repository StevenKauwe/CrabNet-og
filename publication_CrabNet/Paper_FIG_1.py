...
formula_of_interest = (
    "Ba0.05718702 Al0.07727975 Zn0.05873261 P0.11437403 O0.3431221 F0.34930448"
)
...
def plot_all_heads(data_loader, attn_mat, idx=0, layer=0, mask=True):
    ...
    if plot_four:
        ...
        plt.savefig(
            f"figures/Figure1_{mat_prop}_attention_plot_{formula_of_interest}.png",
            bbox_inches="tight",
            dpi=300,
        )
        plt.show()
...
idx = [i for i, out in enumerate(data_loader.dataset) if out[2] == formula_of_interest]
if len(idx) > 0:
    print(f"Found {formula_of_interest} at index {idx}")
